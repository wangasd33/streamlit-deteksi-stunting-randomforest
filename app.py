...
def get_chart_font(size=48, bold=False):
    """
    Memuat font TrueType berukuran besar agar jelas terbaca saat di-deploy di Streamlit Cloud maupun Windows.
    """
    if bold:
        candidates = [
...
def create_growth_chart_with_marker(jenis_kelamin, usia_bulan, tinggi_cm, prediksi_label):
    """
    Membuat gambar grafik WHO dengan overlay titik posisi anak berukuran besar & jelas.
    """
    ...
    font_bold = get_chart_font(size=54, bold=True)
    font_reg = get_chart_font(size=46, bold=False)
    ...
    text_h = h1 + h2 + (pad_y * 2) + 12
    
    label_x = px + dot_radius + 20
    label_y = py - dot_radius - 30
    
    if label_x + text_w > img.width - 20:
        label_x = px - dot_radius - text_w - 20
    if label_y < 20:
    ...
    draw.text((label_x + pad_x, label_y + pad_y - 4), label_text, fill=outline_color, font=font_bold)
    draw.text((label_x + pad_x, label_y + pad_y + h1 + 10), label_info, fill=(30, 41, 59, 255), font=font_reg)
